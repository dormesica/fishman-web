 ...
 */
var Npm = function (fileSystem, incDeps, incDevDeps, incTypes, cbUpdate) {
	Provider.prototype.constructor.call(this, fileSystem, incDeps, incDevDeps, incTypes, cbUpdate);
};
 ...
 */
Npm.prototype.getPackagePath = function (module, ver, path) {
 ...
 */
Npm.prototype.getPackageFolder = function (path) {
	return this.fileSystem.join(path, "packages");
};

Npm.prototype.getTypesModuleIfExist = function (module, ver, path, callback) {
	...
	self.cloneSingleModule(moduleTypeName, ver, path, function(err, ver) {
		if (err) {
			if (err.statusCode === '404')
				self.cbUpdate('regularUpdate',{message: `module for typescript not found, name:${moduleTypeName} version:${ver}`, color: "gray"});
			else
				self.cbUpdate('regularUpdate',{message: `types for module ${module}:${ver} error: ${err.message}`, color: "gray"});
		} 
 ...
 */
Npm.prototype.isModuleExist = function (module, ver, path, cb) {
	var self = this;
 ...
 */
Npm.prototype.cloneSingleModule = function (module, ver, path, callback) {
	var self = this;
	...
	async.waterfall([
		...
		function (pkg, callback) {
			...
			self.fileSystem.mkdir(self.getPackageFolder(path), function(err) {
				...
				self.fileSystem.writeFile(self.getPackagePath(module, ver, path), JSON.stringify(packageJson, null, 4), callback);
			});
			
		},
		function (callback) {
            self.cbUpdate('regularUpdate',{message: 'downloading tarball of ' + module + '-' + ver, color: "gray"});
			var file = self.fileSystem.createWriteStream(self.fileSystem.join(path, module + '-' + ver + '.tgz'));
