...
var cloneModule = function (options, fileSystem, streamToWriteTo, cbUpdate) {
	...
	if (!provider) {
		...
		let moduleFailed = 0
		async.each(options.modules, function(module, cbFinish) {
			...
			async.waterfall([
				...
				if (err) {
					cbUpdate('regularUpdate',{message: `failed to cloned ${module.name} ${err.toString()}`, color: "red"});
					moduleFailed ++;
				}
